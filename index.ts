...
async function rainbowlights() {
  const animationRainbow = chalkAnimation.rainbow(
    "let's Calaculate the values!"
  );
  await promise();
...
async function askQuestion() {
  ...
  if (answers.operator == "Addition") {
    console.log(
      `${answers.Operand1} + ${answers.Operand2} = ${
        answers.Operand1 + answers.Operand2
      }`
    );
  }
  ...
  if (answers.operator == "Exponent") {
    console.log(
      `  The power of (${answers.Operand1} raise to the power  ${
        answers.Operand2
      } = ${answers.Operand1 ** answers.Operand2})`
    );
  }
